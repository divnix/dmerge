      ...
      f = attrPath:
        zipAttrsWith (n: values:
          ...
          then
            ...
            else if isList lhs' && isFunction rhs' then let
              ex = tryEval (rhs' lhs' here');
            in if ex.success then ex.value else abort ''
  ...
  in {
    ...
    in
      ...
      assert assertMsg (length indices == length updates) ''
        UPDATING ARRAY MERGE: for each index there must be one corresponding update value, got: ${traceSeqN 1 indices "(see first trace above)"} indices & ${traceSeqN 1 updates "(see second trace above)"} updates'';
      let
        updated = listToAttrs (
